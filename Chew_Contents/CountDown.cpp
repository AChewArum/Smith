	...
	for (auto& _UI : m_UIList)
	{
		if (_UI->GetUIData()->Data.find(to_wstring(0)) != wstring::npos&&_UI->GetUIData()->Type==L"Close")
		{
			...
			else
			{
				...
				SoundManager::GetInstance()->SFXPlay("Minigame_Success", 9);
...
void CountDown::SetPlaySFX()
{
	wstring which = m_pGameObject->GetComponent<UI>()->GetUIList()[0]->GetUIData()->UIType.substr(0, 4);
