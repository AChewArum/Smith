	...
	: Scene(sceneName)
{
	...
	m_Warrior->AddComponent<Transform>();
	m_Warrior->GetComponent<Transform>()->SetLocalPosition({ 9.5f, 0.f, -2.5f });
...
void TestScene::Update()
{
	...
	for (auto& obj : m_GameObjectList)
	{
		if (obj != nullptr)
		{
			...
			if (obj->GetName().find(L"bubble")!=wstring::npos && obj->GetName() != L"Request_bubble")
			{
			...
			{
				obj->GetComponent<Transform>()->SetLocalPosition({ m_Warrior->GetComponent<Transform>()->GetWorldPosition().x,  m_Warrior->GetComponent<Transform>()->GetWorldPosition().y + 3.f ,  m_Warrior->GetComponent<Transform>()->GetWorldPosition().z });
			}
...
void TestScene::LoadObject(vector<MeshBuildInfo*> MeshInfoList, shared_ptr<ObjectDataTable> ObjData)
{
	...
	for (auto& _MeshInfo : MeshInfoList)
	{
		...
		if (_MeshInfo->IDName == L"Smith")
		{
			...
			_NewGameObject->GetComponent<Movement>()->SetMoveSpeed(0.08f);
		...
		if (_NewGameObject->GetName().find(L"bubble") != wstring::npos)
		{
			_NewGameObject->GetComponent<Transform>()->SetLocalRotate({ PI / 4, -PI / 4 , 0 });
			_NewGameObject->GetComponent<Transform>()->SetWorldScale({ 2.f, 2.f , 2.f });
