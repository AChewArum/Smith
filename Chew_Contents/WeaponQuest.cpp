...
{
	SceneManager::GetInstance()->m_HimeEngine->SetFireEffect(false);
	...
	for (auto& _UI : m_UIList)
	{
		...
		if (CalculateSatistification() == L"GG")
		{
			SoundManager::GetInstance()->SFXPlay("Result_Weapon_Good", 12);
		}
		else
		{
			SoundManager::GetInstance()->SFXPlay("Result_Weapon_Bad", 11);
		}
