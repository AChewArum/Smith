...
void HimeRenderer::Update(HimeCamera* pCamera, float deltaTime, MeshBuildInfo* pMeshInfo)
{
	...
	for (auto _DXObject : m_RenderingQueue)
	{
		...
		if (_DXObject->m_NodeName == pMeshInfo->NodeName)
		{
			...
			XMVECTOR _s = XMLoadFloat4(&pMeshInfo->Scale);
			...
			{
				_DXObject->m_MoveRot = Vector3{ _r.m128_f32[0],_r.m128_f32[1] + (180 * Deg2Rad) ,_r.m128_f32[2] };
			}
			else if (_DXObject->m_ID == pMeshInfo->IDName)
			{
				_DXObject->m_MoveRot = Vector3{ _r.m128_f32[0],_r.m128_f32[1] ,_r.m128_f32[2] };
				if (pMeshInfo->IDName.find(L"bubble")!=wstring::npos)
				{
					_DXObject->m_MoveScale = Vector3{ _s.m128_f32[0],_s.m128_f32[1] ,_s.m128_f32[2] };
				}
			}
			...
			if (pMeshInfo->ObjType == L"Bubble" || pMeshInfo->ObjType == L"Warrior")
			{
