...
void MeshObject::Update(HimeCamera* pCamera)
{
	...
	XMMATRIX _ScaleTM = XMMatrixScalingFromVector(m_MoveScale);

	m_NodeData.m_LocalTM = m_NodeData.m_LocalTM * _RotTM * _MoveTM;
	if (m_NodeName.find(L"bubble") != wstring::npos)
	{
		 XMStoreFloat4x4(&m_NodeData.m_LocalTM, _ScaleTM * _RotTM * _MoveTM);
	}
